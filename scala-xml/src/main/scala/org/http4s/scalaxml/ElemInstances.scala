...
trait ElemInstances {
  implicit def elemEncoder(implicit mediaTypeOf: MediaTypeOf[Elem], charset: Charset = Charset.`UTF-8`): EntityEncoder[Elem] =
    EntityEncoder.stringEncoder(charset)
      .contramap[Elem](xml => xml.buildString(false))
      .withContentType(`Content-Type`(mediaTypeOf.mediaType, charset))
